export default {
  launching: "Launching Early 2026",

  dynamicPhrases: [
    "Find the gear that fits your ride.",
    "Smart picks for smarter riders.",
    "Style and safety matched at perfection.",
    "Your road, your look, your gear.",
    "Ride smarter. Dress better.",
    "Gear made for how you ride."
  ],

  description:
    "VXGear uses AI to analyze your style, your bike and the road conditions to recommend jackets, helmets, gloves and accessories that truly fit you.<br>Be the first to know. Join VXGear and we'll notify you as soon as the app is available. ",

  placeholder: "Enter your email",
  join: "Join now",
  success: "You’re in! Welcome to VXGear.",
  fail: "Something went wrong. Try again.",
  ridersJoined: "Hundreds of riders already joined",

  footer1: "© 2026 — VXGear. All rights reserved.",
  footer2: "Powered by AI. Crafted for Riders.",

  switchToEn: "EN",
  switchToEs: "ES",
};
